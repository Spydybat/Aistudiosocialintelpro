...
export default function DownloadHistory() {
  ...
  const [records, setRecords] = useState<DownloadHistoryRecord[]>([]);
  ...
  const loadHistory = useCallback(async () => {
    ...
    const { data, error } = await fetchDownloadHistory(user.id);
    if (error) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("download-history:updated", handleHistoryUpdated);
    return () => {
      window.removeEventListener("download-history:updated", handleHistoryUpdated);
    };
  }, [loadHistory]);
  ...
  const handleDelete = async (id: string) => {
    const { error } = await deleteDownloadHistory(id);
    if (error) {
      setError(error.message);
      return;
    }
  ...
  const handleClearAll = async () => {
    ...
    const { error } = await clearDownloadHistory(user.id);
    if (error) {
      setError(error.message);
      return;
    }
  ...
  return (
    ...
    >
      <div className="border-b border-zinc-200 dark:border-zinc-900 pb-5 flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold tracking-tight">Download History</h1>
          <p className="text-xs text-zinc-500 mt-1">
            Review saved download records from the existing Supabase database.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <label className="flex items-center gap-2 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950/40 px-3 py-2 text-sm w-full sm:max-w-md">
          ...
          <input
            ...
            placeholder="Search platform, URL, file, status"
      ...
      {loading ? (
        ...
        <div className="overflow-hidden rounded-2xl border border-zinc-200 dark:border-zinc-900 bg-white dark:bg-zinc-950/30">
          <div className="grid grid-cols-[1.1fr_1.6fr_1.1fr_0.8fr_0.8fr_0.8fr_0.3fr] gap-2 p-3 text-[11px] font-bold uppercase tracking-wider text-zinc-500 border-b border-zinc-200 dark:border-zinc-900">
            <span>Platform</span>
            <span>URL</span>
            <span>File Name</span>
            <span>File Size</span>
          ...
          {filteredRecords.map((record) => (
            <div
              key={record.id}
              className="grid grid-cols-[1.1fr_1.6fr_1.1fr_0.8fr_0.8fr_0.8fr_0.3fr] gap-2 p-3 border-t border-zinc-200 dark:border-zinc-900 text-sm items-center"
            >
              <span className="truncate font-medium">{record.platform ?? "—"}</span>
              <span className="truncate text-zinc-600 dark:text-zinc-300">{record.url ?? "—"}</span>
              <span className="truncate">{record.file_name ?? "—"}</span>
              <span className="truncate">{record.file_size ?? "—"}</span>
              <span className="truncate capitalize">{record.status ?? "completed"}</span>
